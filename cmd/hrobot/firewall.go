...
func getFirewall(ctx context.Context, client *hrobot.Client, serverID hrobot.ServerID) error {
	return listRules(ctx, client, serverID, "", "")
}
...
func allowIP(ctx context.Context, client *hrobot.Client, serverID hrobot.ServerID, ipAddr string) error {
	...
	ipVersion := detectIPVersion(ipAddr)
	...
	if ruleExists(fw.Rules.Input, newRule) {
		fmt.Printf("⊘ rule already exists for IP %s, no changes made\n", ipAddr)
		return nil
	}

	fmt.Printf("adding IP %s to allow list...\n", ipAddr)

	// Filter auto-added mail rules and add the new rule to the beginning
	filteredInput := filterAutoAddedRules(fw.Rules.Input)
	updatedRules := append([]hrobot.FirewallRule{newRule}, filteredInput...)
